...
model_ad1 <- advice_data %>% 
  glm(liferefer ~ refer_expo + betweeness + relevel(rolelab2, ref = 3), data = ., family =binomial())
summary(model_ad1)

model_ad2 <- advice_data %>% 
  glm(lifeaware ~ refer_expo + betweeness + rolelable, data = ., family =binomial())
  summary(model_ad2)

#Discussion Network Models - betweenness is sig.
model_dis1 <- discuss_data %>% 
  glm(liferefer ~ refer_expo +  betweeness + relevel(rolelable, ref = "Nurse")  , data = ., family =binomial())
summary(model_dis1)
...
col_role <- c("lightsteelblue", "tomato", "palegreen3", "orange2", "palegoldenrod")
...
col_role2 <- c("lightsteelblue", "tomato", "palegreen3", "orange2",  "palegoldenrod")
...
plot(
  ...
  legend("bottomleft", legend =c("Physician", "Nurse Practioner/Physician Assistant", "Clinic Nurse", "Social Worker", "Scheduler/other"), pch=21,
  col="#777777", pt.bg=col_role, pt.cex=1, cex=1.2, bty="n", ncol=1)
  
  plot(
  discuss_graph, 
  vertex.size     = (degree(discuss_graph)/2 +5),
  #vertex.label    = V(discuss_graph)$id,
  vertex.label = bet2, 
  vertex.label.cex = 1,
  vertex.label.color = c("black"),
  vertex.label.family="Helvetica",
